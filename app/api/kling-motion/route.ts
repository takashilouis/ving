                ...
                try {
                    ...
                    if (task?.task_status === "succeed") {
                        ...
                        if (videoResult?.url) {
                            ...
                            await deductCredits(user.id, requiredCredits, "kling_motion", {
                                prompt: prompt || "Motion Control Video",
                                videoId: videoResult.id,
                            });

                            // Download video and upload to R2 for persistence
                            let r2Url = videoResult.url;
                            let r2Key: string | null = null;

                            try {
                                const { downloadFile, uploadToR2, generateR2Filename } = await import("@/lib/r2-storage");
                                const { buffer, contentType } = await downloadFile(videoResult.url);
                                const filename = generateR2Filename("kling", "mp4");

                                const { url, key } = await uploadToR2(buffer, filename, contentType);
                                r2Url = url;
                                r2Key = key;
                            } catch (r2Error) {
                                console.error("R2 upload failed, using original URL:", r2Error);
                            }
                            ...
                            try {
                                ...
                                console.error("Failed to save Kling video to database:", dbError);
                            }
